...
def main():
    ...
    final_cfg = dict(
        ...
        # NOTE: drop class_weight="balanced" to avoid hard push to extremes
        random_state=777,
        verbose=-1
    )


    # ---- OOF from *the same family* as final model (alignment!)
    ...
    # fallback if all folds skipped (unlikely with your data)
    if not fold_models:
    ...
    # (optional) keep fold models if you want bagging later
    ...
    # --- DIAG 1: how much do features vary across hexes per-hour?
    key_feats = [c for c in ["lag_1","lag_2","lag_3","roll_mean_3","precip_mm","temp_c","osm_density","lat","lon"] if c in fut_feat.columns]
    print("\n[diag] per-hour std (first 5 hours):")
    for ts, g in list(fut_feat.groupby("time"))[:5]:
        stds = g[key_feats].std(numeric_only=True)
        print(ts, "→", stds.round(4).to_dict())

    # --- DIAG 2: lags distribution in the last few history hours
    hist_tail = df.sort_values("time").groupby("h3", sort=False).tail(6)
    if set(["lag_1","lag_2","lag_3","roll_mean_3"]).issubset(df.columns):
        print("\n[diag] history lag_1 nonzero ratio (last 6hrs):", (hist_tail["lag_1"]!=0).mean().round(4))
    else:
        print("\n[diag] no lag columns on df (as expected pre-merge)")

    # --- DIAG 3: raw fold predictions uniqueness
    Xf_df = fut_feat[feats]
    raw_folds = np.column_stack([m.predict_proba(Xf_df)[:, 1] for m in fold_models])
    raw = raw_folds.mean(axis=1)
    print("\n[diag] unique raw preds (rounded 6dp):", np.unique(np.round(raw, 6)).shape[0])
    ...
    # After merging lags into fut_feat:
    nz_frac = (fut_feat[["lag_1","lag_2","lag_3","roll_mean_3"]] != 0).mean().mean()
    print(f"[diag] future lags nonzero fraction: {nz_frac:.3f}")

    for c in ["lag_1", "lag_2", "lag_3", "roll_mean_3"]:
        fut_feat[c] = fut_feat[c].astype('float64').fillna(df['label'].mean())

    # predict with final → logits → calibrator (aligned!)
    Xf_df = fut_feat[feats]
    # 1) Average fold models to get a smoother raw score
    raw_folds = np.column_stack([m.predict_proba(Xf_df)[:, 1] for m in fold_models])
    raw = raw_folds.mean(axis=1)

    # 2) Map via the *aligned* calibrator (trained on OOF logits from same family)
    logits = logit(raw).reshape(-1, 1)
    cal = joblib.load("data_sample/models/cal_logreg.joblib")
    preds = cal.predict_proba(logits)[:, 1]

    # gentle temp smoothing (keeps ranking, reduces saturation)
    def soften(p, T=1.35, eps=1e-6):
        lg = logit(p, eps)
        return 1.0 / (1.0 + np.exp(-lg / T))
    preds = soften(preds, T=1.35)

    out = fut_feat[["h3", "lat", "lon", "time"]].copy()
    out["risk"] = preds
    # per-hour percentile rank for UI (robust to ties)
    ...
    assert len(out) == len(fut_feat), "Row mismatch train→predict"
    assert out["time"].nunique() == 72, "Expected 72 future hours"
