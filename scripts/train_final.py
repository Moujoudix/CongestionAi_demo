    ...
    if events_df is not None:
        ...
        for col in ("start", "end"):
            if col in events_df.columns:
                events_df[col] = (
                    pd.to_datetime(events_df[col], utc=True)
...
def main():
    ...
    # geo prior (per-hex historical mean)
    hist_prior = (
    ...
    # feature list for both train & inference (numeric only, drop target)
    ...
    # final LightGBM config (rich score granularity, stable)
    final_cfg = dict(
        n_estimators=2000,
        learning_rate=0.03,
        num_leaves=256,
        ...
        max_bin=511,
        feature_fraction=0.9,   # alias of colsample_bytree
        bagging_fraction=0.85,  # alias of subsample
        bagging_freq=1,
        reg_lambda=1.0,
        random_state=777,
        verbose=-1,
    ...
    # bring SAME hex_prior to future; fill with global prior if missing
    ...
    # fill any lag holes with global prior (prevents all-zeros far horizons)
    for c in ["lag_1", "lag_2", "lag_3", "roll_mean_3", "hex_prior"]:
        if c not in fut_feat.columns:
            fut_feat[c] = global_prior
        fut_feat[c] = fut_feat[c].astype("float64").fillna(global_prior)

    # PREDICT: avg folds → logit → calibrator → soft temperature
