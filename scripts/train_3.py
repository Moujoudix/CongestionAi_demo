...
def main():
    ...
    # Add spatial prior (per-hex historical rate) BEFORE choosing features
    hist_prior = (
    ...
    # final model config — more leaves → richer score granularity (no class_weight)
    final_cfg = dict(
        n_estimators=2000,
        learning_rate=0.03,
        num_leaves=256,
        ...
        max_bin=511,
        feature_fraction=0.90,     # col sampling
    ...
    # Merge the same hex_prior into FUTURE features
    fut_feat = fut_feat.merge(hist_prior, on="h3", how="left")

    # Fill any missing lag values (neutral) and ensure numeric dtype
    for c in ["lag_1", "lag_2", "lag_3", "roll_mean_3"]:
        fut_feat[c] = fut_feat[c].astype("float64").fillna(0.0)
    fut_feat["hex_prior"] = fut_feat["hex_prior"].astype("float64").fillna(fut_feat["hex_prior"].mean())

    # --- DIAGNOSTICS (now that lags & hex_prior exist)
    key_feats = [c for c in ["lag_1","lag_2","lag_3","roll_mean_3","precip_mm","temp_c","osm_density","hex_prior","lat","lon"] if c in fut_feat.columns]
    ...
    # Predict with folds → logits → calibrator (aligned!)
    Xf_df = fut_feat[feats]  # feats includes hex_prior now
